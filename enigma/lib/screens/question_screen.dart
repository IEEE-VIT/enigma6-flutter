...
const questionUrl =
		'https://enigma6-backend.herokuapp.com/api/auth/getCurrent/';
...
class _QuestionScreenState extends State<QuestionScreen> {
	...
	Future<q.Payload> _getCurrentQuestion(String uid) async {
		...
		try {
			final response = await http
					.post(questionUrl, headers: {'Authorization': 'Bearer $uid'});
	...
	Future<Profile> _getRankScore(String uid) async {
		final response = await http
				.post(rankScoreUrl, headers: {'Authorization': 'Bearer $uid'});
		print(response.body);
												...
												if (snapshot.hasData) {
													...
													return ListView(
														...
														children: <Widget>[
															...
															QuestionPic(snapshot.data.imgUrl,
																					...
																					submitAnswer(
																						...
																						if (result.isAnswerCorrect ==
																							...
																							setState(() {
																								...
																								currentQuestion =
																										_getCurrentQuestion(uId);
																								rankAndScore =
																										_getRankScore(uId);
																							});
																						} else if (result.isAnswerCorrect ==
																								false) {
																							final snackBar = SnackBar(
																									content: Text(
																											result.payload.howClose));
																							Scaffold.of(context)
																									.showSnackBar(snackBar);
																						}
																					});
														...
														child: Column(
															children: <Widget>[
																RankAndScore(
																		...
																		snapshot.data.payload.user.points
																				.toString()),
																Padding(
																	padding: EdgeInsets.only(
																			top: SizeConfig.safeBlockVertical),
